...
def prestar(libro_id, reserva_id=None):
    ...
    # Si se proporciona una reserva, validar que esté aprobada
    if reserva_id:
    ...
    if request.method == 'POST':
        try:
            ...
            libro.reducir_cantidad()  # Reducir la cantidad disponible
    ...
    return render_template('prestar_libro.html', libro=libro, reserva=reserva)
...
def reservar(libro_id):
    ...
    return render_template('reservar_libro.html', libro=libro)
...
def reservas_pendientes():
    ...
    return render_template('reservas_pendientes.html', reservas=reservas)
